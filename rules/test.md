...
* Focus on high-signal ResultTypes: 50126 = invalid credentials, 50053 = account locked, 50074 = MFA required
* Pair with a successful login query to detect post-spray compromise
...
During hands-on testing in the Log(N) Pacific Cyber Range (April 1, 2026), Rule 01 surfaced account `cdeb4c73...@lognpacific.com` with 23 failed attempts across 3 IPs over 45 minutes.
...
* ResultType 50074 — correct password but MFA blocking (IP: 173.239.254.251)
* ResultType 500121 — repeated MFA push failures across iPhone + Mac
* ResultType 50133 — session invalidated after password change
* Third IP (85.8.130.26) introduced mid-attack

**Assessment:** MFA fatigue attack pattern. Attacker had correct credentials but could not bypass MFA. Three source IPs suggest coordinated attempt across multiple devices or proxies.

**Escalation:** Tier 2 account lockout and credential reset recommended. Password change (50133) suggests account may have been previously compromised.

---

If you want, I can make the **exact same style for Rule 02** that tracks “Successful Login After Failed Attempts,” so your repo will look like a polished professional SOC detection library.
